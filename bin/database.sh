...
check_db_access(){
    docker compose exec -T mysql su -c "mariadb -uroot -p${MYSQL_ROOT_PASSWORD} -e 'status'" >/dev/null 2>&1
    if [ ${?} != 0 ]; then
...
db_setup(){  
    docker compose exec -T mysql su -c 'mariadb -uroot -p${MYSQL_ROOT_PASSWORD} \
